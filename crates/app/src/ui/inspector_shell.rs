...
pub struct InspectorSignals<'a> {
    ...
    pub review_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_audit_packet_review_state::ReviewSummaryRow>>,
    pub distribution_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_audit_packet_distribution_state::DistributionSummaryRow>>,
    pub ladder_result_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_manual_result_state::WorkflowManualResultSummaryRow>>,
    pub ladder_review_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_manual_result_review_state::WorkflowManualResultReviewSummaryRow>>,
    pub ladder_readiness_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_manual_result_reconciliation_readiness_state::WorkflowManualResultReconciliationReadinessSummaryRow>>,
    pub ladder_gate_rows: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_manual_result_reconciliation_gate_state::WorkflowManualResultReconciliationGateSummaryRow>>,
    pub ladder_predicates: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_manual_result_reconciliation_readiness_state::ReadinessPredicateDisplayRow>>,
    pub routing_route_row: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_action_routing_state::WorkflowActionRouteSummaryRow>>,
    pub routing_session_row: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_action_routing_state::WorkflowSessionRouteRow>>,
    pub routing_route_predicates: &'a dioxus::prelude::GlobalSignal<Vec<openwand_app::ui::workflow_action_routing_state::WorkflowActionRoutePredicateRow>>,
    pub routing_route_prompt: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_action_routing_state::WorkflowActionRoutePromptRow>>,
    pub routing_readiness_state: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_routing_readiness_state::WorkflowRoutingReadinessUiState>>,
    pub routing_next_action_state: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_next_action_routing_state::WorkflowNextActionRoutingUiState>>,
    pub routing_review_row: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_next_action_review_state::ReviewSummaryRow>>,
    pub execution_timeline_state: &'a dioxus::prelude::GlobalSignal<Option<openwand_app::ui::workflow_execution_state::WorkflowExecutionUiState>>,
}
...
impl<'a> InspectorSignals<'a> {
    ...
    fn load_manual_result_ladder(&self, path: &std::path::Path, wfx_id: &openwand_workflow::workflow_run::WorkflowExecutionId) {
        if let Ok(Some(mr)) = openwand_app::workflow_manual_result::result_by_workflow_run(path, &wfx_id.0) {
            *self.ladder_result_rows.write() = vec![openwand_app::ui::workflow_manual_result_state::workflow_manual_result_summary_lines(&mr)];
        } else {
            *self.ladder_result_rows.write() = vec![];
        }
        if let Ok(Some(mrr)) = openwand_app::workflow_manual_result_review::review_by_workflow_run(path, &wfx_id.0) {
            *self.ladder_review_rows.write() = vec![openwand_app::ui::workflow_manual_result_review_state::workflow_manual_result_review_summary_lines(&mrr)];
        } else {
            *self.ladder_review_rows.write() = vec![];
        }
        ...
        if let Ok(Some(gate)) = openwand_app::workflow_manual_result_reconciliation_gate::gate_by_workflow_run(path, &wfx_id.0) {
            *self.ladder_gate_rows.write() = vec![openwand_app::ui::workflow_manual_result_reconciliation_gate_state::gate_summary_lines(&gate)];
        } else {
            *self.ladder_gate_rows.write() = vec![];
        }
    }
    ...
    fn load_routing_ladder(&self, path: &std::path::Path, wfx_id: &openwand_workflow::workflow_run::WorkflowExecutionId) {
        if let Ok(Some(route)) = openwand_app::workflow_action_routing::route_by_workflow_run(path, &wfx_id.0) {
            *self.routing_route_row.write() = Some(openwand_app::ui::workflow_action_routing_state::workflow_action_route_summary(&route));
            *self.routing_session_row.write() = openwand_app::ui::workflow_action_routing_state::workflow_session_route_row(&route);
            *self.routing_route_predicates.write() = openwand_app::ui::workflow_action_routing_state::workflow_action_route_predicate_rows(&route);
            *self.routing_route_prompt.write() = Some(openwand_app::ui::workflow_action_routing_state::workflow_action_route_prompt_row(&route));
        } else {
            *self.routing_route_row.write() = None;
            *self.routing_session_row.write() = None;
            *self.routing_route_predicates.write() = vec![];
        ...
        if let Ok(Some(nar)) = openwand_app::workflow_next_action_routing::routing_by_workflow_run(path, &wfx_id.0) {
            let ui_state = openwand_app::ui::workflow_next_action_routing_state::WorkflowNextActionRoutingUiState {
